...
for (i in c(2018, 2020, 2022)){#2018, 2019, 2020, 2021, 2022
  ...
  fla_hog <- base_hogar %>% select(DIRECTORIO,SECUENCIA_P,P5666,P8536,P1698,
  ...
  fla_hog <- base_hogar %>% select(DIRECTORIO,SECUENCIA_ENCUESTA,SECUENCIA_P,P5666,
  ...
  educ <- base_educ %>% select(DIRECTORIO,SECUENCIA_ENCUESTA,SECUENCIA_P,ORDEN,
                               P8587,P8587S1)
  pobl <- base_poblacion %>% select(DIRECTORIO,SECUENCIA_ENCUESTA,SECUENCIA_P,ORDEN,
                                    P6020,P6040,P6051)
  
  base_pop <- inner_join(x = educ,y = pobl,by=c("DIRECTORIO","SECUENCIA_P","ORDEN"))
  base_pop <- base_pop %>% group_by(DIRECTORIO,SECUENCIA_P,ORDEN) %>% mutate(avg_edad=mean(P6040))
  base_pop <- base_pop %>% group_by(DIRECTORIO,SECUENCIA_P,ORDEN) %>% 
    mutate(Jefe_sex=ifelse(P6051==1,P6020,0),Jefe_edad=ifelse(P6051==1,P6040,0),
           menores=ifelse(P6040<=18,1,0),Jefe_educ_sec=ifelse(P8587==4|P8587==5&P6051==1,1,0),
           Jefe_educ_ter=ifelse(P8587>=6&P6051==1,1,0))
  
  ##############################################################################'#
  ##########################  Agrupacion personas  ###############################
  ##############################################################################'#
  
  base_pop_clear <- base_pop %>% summarise(Jefe_sex=max(Jefe_sex),
                                           Jefe_edad=max(Jefe_edad),
                                           menores=sum(menores),
                                           Jefe_educ_sec=max(Jefe_educ_sec),
                                           Jefe_educ_ter=max(Jefe_educ_ter),
                                           avg_edad=mean(avg_edad))
  
  base <- inner_join(base_combustible,base_pop_clear,by=c("DIRECTORIO","SECUENCIA_P",
                                                          "ORDEN"))
